...
class ResNet(object):
    def __init__(self, channels, name, layers=2, activation=tf.nn.relu):
        ...
        for i in range(layers):
            v = {}
            v['conv'] = op.Encoder(channels, channels, 1, 1, name='{}')
            v['activation'] = activation

        self.normalization = op.BatchNormalization(name='{}.bnc'.format(name))
...
class Upsampler(object):
    def __init__(self, channels):
        ...
        self.conv1 = op.Encoder(channels, 64, 3, 3, strides=[1, 2, 2, 1], name='upsampler.encoder0')
        self.conv2 = op.Encoder(64, 128, 4, 4, strides=[1, 1, 1, 1], name='upsampler.encoder1')
        self.conv3 = op.Encoder(128, 256, 4, 4, strides=[1, 2, 2, 1], name='upsampler.encoder2')
        self.conv4 = op.Encoder(256, 512, 4, 4, strides=[1, 1, 1, 1], name='upsampler.encoder3')
...
class Generator(object):
    def __init__(self, channels):
        ...
        self.deconv8 = op.PixelShuffler(op.Encoder(1536, 1024, 3, 3, name='decoder8'), 256, 2)
...
def generator(line_art, small):
    """Make construction layer.
    """
    channels = line_art.shape.as_list()[3]
    gen = Generator(channels)

    relu = tf.nn.relu

    conv0 = relu(gen.bnc0(gen.conv0(line_art)))
    ...
    deconv8 = relu(gen.bnd8(gen.deconv8(tf.concat([small, conv8, conv7], 3))))
