...
def train():
    ...
    with tf.Graph().as_default():
        ...
        with tf.device('/cpu:0'):
            ...
            tags, images = tf_dataset_input.inputs(ARGS.dataset_dir, ARGS.batch_size, ARGS.max_document_length)
            embedding = tf.Variable(tf.random_uniform([len(vocab), EMBEDDING_SIZE], -1.0, 1.0), name="embedding")
        ...
        with tf.name_scope('c_train'):
            training = model.training(loss, ARGS.learning_rate, global_step_tensor, None)
        ...
        sliced_embedding = tf.Variable(
            tf.random_uniform([500, EMBEDDING_SIZE], -1.0, 1.0), trainable=False, name="sliced_embedding")
