...
class AutoEncoder(object):
    ...
    def __init__(self):
        ...
        self.deconv3 = op.PixelShuffler(op.Encoder(512, 1024, 4, 4, name="decoder3"), 256, 2)
...
def autoencoder(images, scale=1.0):
    ...
    relu = tf.nn.relu
    net = relu(AE.bnc0(AE.conv0(images)))
    net = relu(AE.bnc1(AE.conv1(net)))
    net = relu(AE.bnc1_f1(AE.conv1_f1(net)))
    net = l1 = relu(AE.bnc1_f2(AE.conv1_f2(net)))
    net = relu(AE.bnc2(AE.conv2(net)))
    net = relu(AE.bnc2_f1(AE.conv2_f1(net)))
    net = l2 = relu(AE.bnc2_f2(AE.conv2_f2(net)))
    ...
    net = relu(AE.bnd2(AE.deconv2(tf.concat([net, l2], 3))))
    net = relu(AE.bnd2_f1(AE.deconv2_f1(net)))
    net = relu(AE.bnd2_f2(AE.deconv2_f2(net)))
    net = relu(AE.bnd1(AE.deconv1(tf.concat([net, l1], 3))))
...
def loss(target, gen, lossmap):
    ...
    with tf.name_scope('loss'):
        loss = tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square(lossmap * (gen - target)), axis=[1,2,3])))
